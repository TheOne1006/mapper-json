...
export default class MapperJson {
  ...
  pickItem(source: any, ruleItem: any): any {
    ...
    let type =
      typeof ruleItem === 'string'
        ? RULE_OPTION_TYPE_STRING // 字符串
        : Array.isArray(ruleItem)
        ? RULE_OPTION_TYPE_ARRAY
        : _.isObject(ruleItem)
        ? RULE_OPTION_TYPE_MULI
        : ''
  ...
  switchMapTask(source: any, options: any) {
    ...
    if (selectOptions && selectOptions.length) {
      const matchItem = _.find(selectOptions, itemOption => {
        const leftValue = itemOption.leftValue || _.result(source, itemOption.leftSelect)
        const rightValue = itemOption.rightValue || _.result(source, itemOption.rightSelect)
        return operationMatch(leftValue, itemOption.operation, rightValue)
      })

      if (matchItem) {
        result = matchItem.result || _.result(source, matchItem.resultSelect)
      }
    }
  ...
  str2numTask(source: any, options: any = {}) {
    ...
    const value = _.toNumber(_.result(source, select))
